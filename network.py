...
st.set_page_config(page_title="PRTG Bandwidth – FINAL", layout="wide")
...
def get_perfect_peaks(sensor_id):
    ...
    try:
        ...
        for ch in data.get("channels", []):
            ...
            mbps = float(raw) / 1_000_000                  # ← THIS IS THE TRUTH
...
st.title("PRTG Bandwidth – 100 % Correct (Finally!)")
st.caption(f"Period: **{period}**")

total_in = total_out = 0.0
for name, sid in SENSORS.items():
    i, o = get_perfect_peaks(sid)
    total_in  += i
    total_out += o
...
st.success(f"**TOTAL PEAK → {total_in:,.2f} Mbps In • {total_out:,.2f} Mbps Out**")
...
for i, v in enumerate([total_in, total_out]):
    ax.text(i, v*1.02, f"{v:,.2f}", ha="center", fontweight="bold", color="white", fontsize=16)
st.pyplot(fig)
