...
# Update this to match Pioneer's actual total uplink capacity
TOTAL_CAPACITY = 40000  # Mbps (40 Gbps)

# Unique key to force selectbox reset on reruns if needed
...
def get_graph_image(sensor_id, graphid):
    ...
    params = {
        "id": sensor_id,
        "graphid": graphid,
        "width": 2200,           # wider → better readability
        "height": 1000,          # taller → more space for legend & labels
        "bgcolor": "f8f9fa",     # very light gray – clean & readable
        "fontcolor": "212529",   # near-black text
        "gridcolor": "dee2e6",   # subtle light grid
        "gridlinewidth": "1",
        "legend": "1",           # force legend
        "username": USER,
        "passhash": PH
    }
    try:
        ...
        st.warning(f"Graph fetch failed for sensor {sensor_id}: {e}")
        return None

# ────────────────────────────────────────────────
st.title("PRTG Bandwidth Dashboard")
st.caption(f"Selected period: **{period}**   |   Total capacity: **{TOTAL_CAPACITY:,.0f} Mbps**")
