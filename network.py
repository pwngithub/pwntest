...
st.set_page_config(page_title="PRTG Bandwidth – Pixel-Perfect Accuracy", layout="wide", page_icon="Signal")
...
def get_accurate_peaks(sensor_id):
    ...
    try:
        ...
        for ch in data.get("channels", []):
            ...
            if not max_raw or float(max_raw) == 0:
                continue
            # CORRECT FORMULA: bytes in 10-second interval → Mbps
            mbps = (float(max_raw) * 8) / (10 * 1_000_000)
...
st.title("PRTG Bandwidth – 100 % Accurate (Finally!)")
...
for i in range(0, len(SENSORS), 2):
    ...
    for col, (name, sid) in zip(cols, pair):
        with col:
            i_peak, o_peak = get_accurate_peaks(sid)
            total_in  += i_peak
            total_out += o_peak
...
bars = ax.bar(["Total Peak In", "Total Peak Out"], [total_in, total_out],
              color=["#00ff88", "#ff3366"], width=0.6, edgecolor="white", linewidth=1.5)
ax.set_ylabel("Mbps", color="white", fontsize=14)
ax.set_title(f"Total Peak Bandwidth ({period})", color="white", fontsize=18, fontweight="bold")
...
for bar in bars:
    ...
    ax.text(bar.get_x() + bar.get_width()/2., height*1.02,
            f"{height:,.2f}", ha="center", va="bottom", fontweight="bold", color="white", fontsize=16)
st.pyplot(fig)
