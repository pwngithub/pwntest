def run_preps_dashboard():
    ...
    def fetch_prep_data():
        url = (
            "https://api.jotform.com/form/210823797836164/"
            "submissions?apiKey=22179825a79dba61013e4fc3b9d30fa4&limit=1000"
        )
    ...
    default_start = (
        max(min_date, max_date - _dt.timedelta(days=6))
    ...
    selected_techs = st.sidebar.multiselect(
        "Select Tech(s)", sorted(filtered_df["Tech"].dropna().unique())
    )
    selected_drops = st.sidebar.multiselect(
        "Select Drop Size(s)", sorted(filtered_df["Drop Size"].dropna().unique())
    )
    ...
    preps_per_tech = (
        filtered_df.groupby("Tech").size().reset_index(name="Prep Count")
    )
    chart2 = (
        alt.Chart(preps_per_tech)
        .mark_bar()
        .encode(
            x="Tech:N",
            y="Prep Count:Q",
    ...
    stacked_df = (
        filtered_df.groupby(["Tech", "Drop Size"])
        .size()
    ...
    # This is the one you mentioned: Prep Trend Over Time
    line_chart = (
        alt.Chart(preps_over_time)
    ...
    # 2) Then tables under the trend chart
    st.markdown("---")
    st.subheader("📋 Filtered Results")
    ...
    summary = (
        filtered_df.groupby(["Date", "Tech", "Drop Size"])
        .size()
        .reset_index(name="Count")
    )
